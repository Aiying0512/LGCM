...
x <- z
C1 <- runif(p1,min = 0.30,max =0.85 )
C1r <- round(C1,digits = 2)
C2 <- runif(p1,min = 1.5, max =3)
...
for(i in 1:p1){
  for (j in (p1+1):p){
    ...
    for (k in 1:(n-1)){
      for (l in (k+1):n){
        a = 2*(x[k,i]-x[l,i])*sign(x[k,j]-x[l,j])/(n*(n-1));
        R1[i,j] = a + R1[i,j];
      }
    }
  }
...
R1[(p1+1):p,(p1+1):p]  <- cor(x[,(p1+1):p])
R1[lower.tri(R1)] <- t(R1)[lower.tri(t(R1))]
...
trans_con <- function(x,delta1){
  corr = matrix(c(1,x/sqrt(2),x/sqrt(2),1),2,2,byrow = T)
  pb <- pmvnorm(lower = c(delta1,0), upper = c(Inf,Inf), corr = corr) 
  pb[1]
}
...
for(i in 1:(p1-1)){
  for (j in (i+1):(p1)){
    ...
    u <- R1[i,j]/2 + (pnorm(-delta_1[i])+pnorm(-delta_2[i]))*(pnorm(-delta_1[j])+pnorm(-delta_2[j]))
    fun <- function(x) {
...
for(i in 1:p1){
  for (j in (p1+1):p){
    ...
    u <- R1[i,j]/4 + (pnorm(-delta_1[i])+pnorm(-delta_2[i]))/2
    fun <- function(x) {
      trans_con(x,delta_1[i]) + trans_con(x,delta_2[i]) - u
    }
...
Pho[(p1+1):p,(p1+1):p] <-R1[(p1+1):p,(p1+1):p]
